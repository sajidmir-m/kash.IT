  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const match = list.find(c => (c.name || '').toLowerCase() === (displayName || '').toLowerCase());
        if (!match) { setProducts([]); setLoading(false); return; }
        // 2) fetch products by category_id
        const res = await api.get('/api/products', { params: { category_id: match.id, per_page: 100 } });
