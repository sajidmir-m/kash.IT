...
class Vendor(db.Model):
    ...
    business_type = db.Column(db.String(100))  # Individual, Company, etc.
    ...
    assigned_categories = db.relationship('VendorCategory', backref='vendor', lazy=True, cascade='all, delete-orphan')
...
class VendorCategory(db.Model):
    ...
    __table_args__ = (db.UniqueConstraint('vendor_id', 'category_id', name='unique_vendor_category'),)
...
class Product(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    category_id = db.Column(db.Integer, db.ForeignKey('categories.id'), nullable=False)
    vendor_id = db.Column(db.Integer, db.ForeignKey('vendors.id'), nullable=True)  # NULL for admin products
    ...
    image_url = db.Column(db.String(500))
    is_active = db.Column(db.Boolean, default=True)
    is_approved = db.Column(db.Boolean, default=True)  # Admin products are auto-approved
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class Order(db.Model):
    ...
    delivery_status = db.Column(db.String(50), default='Pending')  # Pending, Assigned, Out for Delivery, Delivered
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
