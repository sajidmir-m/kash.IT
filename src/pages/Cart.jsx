...
export default function Cart() {
  ...
  const onPlaceOrder = async () => {
    ...
    try {
      // Ensure backend cart matches client cart before creating order
      const syncServerCart = async () => {
        ...
        for (const item of cartItems) {
          try {
            ...
            throw new Error(e?.message || 'Failed to sync cart');
          }
        }
      };
      ...
      // If Cash on Delivery, place order directly without Razorpay
      ...
      // 1) Create Razorpay order on backend (fallback to COD if payment is not configured)
      let key, order;
      try {
        const res = await createRazorpayOrder({ amount: toPay, currency: 'INR', receipt: `rcpt_${Date.now()}` });
        key = res.key; order = res.order;
      } catch (e) {
        alert((e?.response?.data?.error || e.message || 'Online payment unavailable') + '\nFalling back to Cash on Delivery.');
        const payload = {
          address_id: defaultAddressId,
          coupon_code: couponApplied?.code || undefined,
          payment_method: 'COD'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-4">
              ...
              {!hasAddress ? (
                ...
                >
                  <Wallet className="w-4 h-4" /> {paymentMethod === 'COD' ? 'Place Order (COD)' : 'Click to pay'} • ₹{toPay}
                </button>
              )}
